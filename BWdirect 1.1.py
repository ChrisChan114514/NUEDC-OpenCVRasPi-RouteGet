...
def simplify_path(points, epsilon):
    ...
    for i in range(1, len(points) - 1):
        point = points[i]
        distance = dist.point_to_line(point, start_point, end_point)
    ...
    if max_distance > epsilon:
        left_points = simplify_path(points[:index + 1], epsilon)
        right_points = simplify_path(points[index:], epsilon)
...
def scan_contour_for_points(frame, contour, start_point, num_scans=10):
    if contour is not None:
        ...
        # 使用Ramer-Douglas-Peucker算法简化路径
        simplified_points = simplify_path(sorted_points, epsilon=10.0)

        # 用绿色线连接简化后的路径点
        if len(simplified_points) > 1:
            for i in range(1, len(simplified_points)):
                cv2.line(frame, simplified_points[i - 1], simplified_points[i], (0, 255, 0), 2)  # 绿色线条
