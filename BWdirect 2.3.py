#版本说明：V1.2 增加标点顺序可视化
#V2.2 改变找出有效黑线向量后的巡线逻辑，模仿红外四线进行巡线（但是向量）
#V2.3 增加连点就近原则
import cv2
import numpy as np
...
def is_near_edge(point, frame_shape, edge_threshold=20):
    """判断点是否靠近图像边缘"""
    x, y = point
...
def scan_row(frame, contour, y, previous_mid_point):
    ...
    if current_mid_points:
        ...
        # 如果中点靠近边缘，返回 None
        if is_near_edge(closest_mid_point, frame.shape):
            return None
...
def scan_contour_for_points(frame, contour, start_point, num_scans=10):
    if contour is not None:
        ...
        # 从第一个点开始，寻找最相近的点进行连接
        sorted_points = mid_points[1:]  # 从第一个点开始
        ...
        for idx, point in enumerate(sorted_points):
            cv2.putText(frame, str(idx), point, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)  # 蓝色序号
...
def draw_boundary_lines(frame, first_mid_point):
    ...
    if first_mid_point is not None:
        ...
        left_boundary = center_x - 20
        right_boundary = center_x + 20
        # 绘制左边界
...
def check_path_and_boundary(sorted_points, frame):
    """判断路径与边界的关系，并输出调整信息"""
    height, width = frame.shape[:2]
...
def main():
    ...
    while True:
        ...
        sorted_points = scan_contour_for_points(frame, path_contour, start_point, num_scans=10)

        # 绘制巡线边界
        if len(sorted_points) > 0:
            first_mid_point = sorted_points[0]
            draw_boundary_lines(frame, first_mid_point)

        # 判断路径与边界的关系，并输出调整信息
        adjustment = check_path_and_boundary(sorted_points, frame)
